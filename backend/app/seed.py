...
def init_db():
    conn = connect()
    conn.executescript("""
    CREATE TABLE IF NOT EXISTS rooms(id INTEGER PRIMARY KEY, name TEXT, length REAL, width REAL, height REAL, damp INTEGER NOT NULL DEFAULT 0);
    ...
    """)
    # 老库迁移：补 rooms.damp 列与默认潮湿系数
    cols = [r["name"] for r in conn.execute("PRAGMA table_info(rooms)").fetchall()]
    if "damp" not in cols:
        conn.execute("ALTER TABLE rooms ADD COLUMN damp INTEGER NOT NULL DEFAULT 0")
    conn.execute("INSERT OR IGNORE INTO settings(key,value) VALUES ('damp_factor','1')")
    conn.commit()
